...
def collate(examples: List[Dict], config: VerticalAttentionTableBertConfig, train=True) -> Dict[str, torch.Tensor]:
    ...
    max_sequence_len = max(len(row["token_ids"]) for e in examples for row in e["rows"])
    max_context_len = max(row["context_span"][1] - row["context_span"][0] for e in examples for row in e["rows"])
    ...
    context_token_positions = np.zeros((batch_size, max_row_num, max_context_len), dtype=int)
    ...
    column_token_position_to_column_ids = np.full(
        (batch_size, max_row_num, max_sequence_len), dtype=int, fill_value=column_token_to_column_id_fill_val
    )
    ...
    if train:
        ...
        if predict_cell_tokens:
            max_masked_cell_token_num = max(
                len(row["masked_cell_token_positions"]) for e in examples for row in e["rows"]
            )
            ...
            masked_cell_token_label_ids = np.full(
                (batch_size, max_row_num, max_masked_cell_token_num), dtype=np.int64, fill_value=-1
            )
    ...
    for e_id, example in enumerate(examples):
        for row_id, row_inst in enumerate(example["rows"]):
            ...
            row_context_token_positions = list(range(row_inst["context_span"][0], row_inst["context_span"][1]))
            ...
            cur_column_num = (
                row_column_token_position_to_column_ids[
                    row_column_token_position_to_column_ids != column_token_to_column_id_fill_val
                ].max()
                + 1
            )
            row_column_nums.append(cur_column_num)
            column_token_position_to_column_ids[e_id, row_id, : len(row_column_token_position_to_column_ids)] = (
                row_column_token_position_to_column_ids
            )

            if train and predict_cell_tokens:
                row_masked_cell_token_positions = row_inst["masked_cell_token_positions"]
                masked_cell_token_positions[e_id, row_id, : len(row_masked_cell_token_positions)] = (
                    row_masked_cell_token_positions
                )
                ...
                masked_cell_token_label_ids[e_id, row_id, : len(row_masked_cell_token_positions)] = row_inst[
                    "masked_cell_token_label_ids"
                ]
        ...
        if train:
            masked_context_token_label_ids[e_id, example["masked_context_token_positions"]] = example[
                "masked_context_token_label_ids"
            ]
    ...
    column_token_position_to_column_ids[column_token_position_to_column_ids == column_token_to_column_id_fill_val] = (
        max_column_num
    )
