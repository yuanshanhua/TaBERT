...
from ..input_formatter import VanillaTableBertInputFormatter
...
class VerticalAttentionTableBertInputFormatter(VanillaTableBertInputFormatter):
    ...
    def get_pretraining_instances_from_example(self, example: Example, context_sampler: Callable):
        instances = []
    ...
    def create_pretraining_instance(self, context: List[str], table: Table, example: Example):
        ...
        for row_id, row_instance in enumerate(row_instances):
            maskable_column_token_indices = [
                (list(range(*span["column_name"])) + list(range(*span["type"])))
                for col_id, span in enumerate(row_instance["column_spans"])
            ]

            masked_column_token_indices = [
                token_idx for col_id in columns_to_mask for token_idx in maskable_column_token_indices[col_id]
            ]

            masked_cell_token_indices = [
                range(*row_instance["column_spans"][col_id]["value"]) for col_id in columns_to_mask
            ]
            ...
            if row_id == 0:
                masked_column_token_column_ids = [
                    col_id for col_id in columns_to_mask for token_idx in maskable_column_token_indices[col_id]
                ]
        ...
        context_token_indices = (
            ...
            else list(range(*row_instances[0]["context_span"]))[:-1]
        )

        num_context_tokens_to_mask = min(
            max_context_token_to_mask, max(1, int(len(context_token_indices) * self.config.masked_context_prob))
        )
        ...
        for row_id, row_instance in enumerate(row_instances):
            masked_token_indices_list.append(masked_context_token_indices + masked_column_token_indices_list[row_id])
        ...
        if self.config.predict_cell_tokens and all(
            len(masked_cell_token_indices_list[i]) == 0 for i in range(len(row_instances))
        ):
            return None
            ...
            "rows": [
                {
                    ...
                    "masked_cell_token_positions": masked_cell_token_indices_list[row_id]
                    if self.config.predict_cell_tokens
                    else None,
                    "masked_cell_token_label_ids": self.tokenizer.convert_tokens_to_ids(
                        masked_cell_token_labels_list[row_id]
                    )
                    if self.config.predict_cell_tokens
                    else None,
                }
