...
from .utils import TRANSFORMER_VERSION, BertForMaskedLM, TransformerVersion
...
class VanillaTableBert(TableBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, **kwargs):
        ...
        if masked_lm_labels is not None:
            ...
            masked_lm_loss = loss_fct(
                prediction_scores.view(-1, self.bert_config.vocab_size), masked_lm_labels.view(-1)
            )

            sample_size = masked_lm_labels.ne(-1).sum().item()
            logging_output = {"sample_size": sample_size, "loss": masked_lm_loss.item()}
    ...
    def encode_context_and_table(
        ...
        **kwargs,
        ...
        kwargs = {} if TRANSFORMER_VERSION == TransformerVersion.TRANSFORMERS else {"output_all_encoded_layers": False}
    ...
    def to_tensor_dict(self, contexts: List[List[str]], tables: List[Table], table_specific_tensors=True):
        ...
        # basic tensors
        input_array = np.zeros((batch_size, max_sequence_len), dtype=int)
        ...
        if table_specific_tensors:
            ...
            context_token_indices = np.zeros((batch_size, max_context_len), dtype=int)
            ...
            column_token_to_column_id = np.zeros((batch_size, max_sequence_len), dtype=int)
        ...
        for i, instance in enumerate(instances):
            ...
            mask_array[i, : len(token_ids)] = 1.0

            if table_specific_tensors:
                context_token_indices[i, : instance["context_length"]] = list(
                    range(*instance["context_span"])
                )  # instance['context_token_indices']
                context_mask[i, : instance["context_length"]] = 1.0
                ...
                for col_id, column in enumerate(header):
                    if col_id < len(instance["column_spans"]):
                        ...
                        column_token_mask[i, col_start:col_end] = 1.0
                        column_mask[i, col_id] = 1.0
    ...
    def encode(
        ...
        context_encoding, column_encoding, encoding_info = self.encode_context_and_table(
            **tensor_dict, return_bert_encoding=return_bert_encoding
        )

        info = {"tensor_dict": tensor_dict, "instances": instances, **encoding_info}
