...
from .table_bert import MAX_BERT_INPUT_LENGTH
...
class VanillaTableBertInputFormatter(TableBertBertInputFormatter):
    def get_cell_input(self, column: Column, cell_value: List[str], token_offset: int):
        input = []
        span_map = {"first_token": (token_offset, token_offset + 1)}

        for token in self.config.cell_input_template:
            start_token_abs_position = len(input) + token_offset
            if token == "column":
                span_map["column_name"] = (
                    start_token_abs_position,
                    start_token_abs_position + len(column.name_tokens),
                )
                input.extend(column.name_tokens)
            elif token == "value":
                span_map["value"] = (start_token_abs_position, start_token_abs_position + len(cell_value))
    ...
    def get_input(self, context: List[str], table: Table, trim_long_table=False):
        row_data = [column.sample_value_tokens for column in table.header]
    ...
    def get_row_input(self, context: List[str], header: List[Column], row_data: List[Any], trim_long_table=False):
        ...
        for col_id, column in enumerate(header):
            ...
            column_input_tokens, token_span_map = self.get_cell_input(
                column, truncated_value_tokens, token_offset=column_start_idx
            )
    ...
    def create_pretraining_instance(self, context, header):
        ...
        column_candidate_indices = [
            (
                ...
                + (span["other_tokens"] if random() < 0.01 and "other_tokens" in span else [])
            )
        ...
        context_candidate_indices = (
            ...
            if self.config.context_first
            else list(range(*input_instance["context_span"]))[:-1]
        )
    ...
    def create_masked_lm_predictions(self, tokens, context_indices, column_indices):
        ...
        if table_mask_strategy == "column_token":
            ...
            num_column_tokens_to_mask = min(
                self.config.max_predictions_per_seq, max(2, int(len(column_indices) * self.config.masked_column_prob))
            )
        ...
        num_context_tokens_to_mask = min(
            max_context_token_to_mask, max(1, int(len(context_indices) * self.config.masked_context_prob))
        )
