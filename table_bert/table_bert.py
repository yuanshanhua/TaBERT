...
class TableBertModel(nn.Module):
    ...
    @classmethod
    def load(
        cls, model_path: Union[str, Path], config_file: Optional[Union[str, Path]] = None, **override_config: Dict
    ):
        ...
        if cls == TableBertModel:
            if "num_vertical_attention_heads" in config_dict:
                from .vertical.vertical_attention_table_bert import (
        ...
        if state_dict is not None:
                ...
                for key in state_dict:
                    if key.endswith(".predictions.bias"):
                        old_key_to_new_key_names.append(
                            (key, key.replace(".predictions.bias", ".predictions.decoder.bias"))
                        )
            ...
            if not any(key.startswith("_bert_model") for key in state_dict):
                ...
                bert_model = BertForMaskedLM.from_pretrained(config.base_model_name, state_dict=state_dict)
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        )  # fix: Unexpected key(s) in state_dict: "_bert_model.bert.pooler.dense.weight", "_bert_model.bert.pooler.dense.bias".

        return model

    def encode(
        self, contexts: List[List[str]], tables: List[Table], **kwargs: Dict
