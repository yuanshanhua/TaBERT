...
class TableDataset(Dataset):
    ...
    def get_dataset_info(cls, data_path: Path, max_epoch=-1):
        if max_epoch == -1:
            ...
            epoch_ids = [int(re.search(r"epoch_(\d+)", str(f)).group(1)) for f in epoch_files]
        ...
        return {"total_size": data_size, "max_epoch": max_epoch}
    ...
    def get_epoch_shards_info(cls, shard_file_prefix: Path):
        ...
        return {"shard_num": shard_num, "total_size": cum_size}
    ...
    def collate(examples):
        ...
        input_array = np.zeros((batch_size, max_len), dtype=int)
        mask_array = np.zeros((batch_size, max_len), dtype=bool)
        segment_array = np.zeros((batch_size, max_len), dtype=bool)
        lm_label_array = np.full((batch_size, max_len), dtype=int, fill_value=-1)
    ...
    def from_dict(cls, entry: Dict, tokenizer: Optional[BertTokenizer], suffix) -> "Example":
        ...
        for col in header_entry:
            ...
            if tokenizer:
                ...
                name_tokens = None
            column = Column(col["name"], col["type"], sample_value, name_tokens=name_tokens)
        ...
        return cls(uuid, header, [context_before, context_after], column_data=column_data, source=source)
...
class TableDatabase:
    ...
    def from_jsonl(
        ...
        if backend == "redis":
            ...
            with file_path.open() as f:
                for idx, json_line in enumerate(
                    tqdm(f, desc=f"Loading Tables from {str(file_path)}", unit="entries", file=sys.stdout)
                ):
                    if indices and idx not in indices:
                        continue

                    example = Example.from_dict(ujson.loads(json_line), tokenizer, suffix=None)
    ...
    def load_data_to_redis(self, file_path: Path):
        reader = multiprocessing.Process(
            target=self.__load_process_zmq, args=(file_path, self.num_workers), daemon=True
        )

        workers = []
        for _ in range(self.num_workers):
            worker = multiprocessing.Process(
                target=self.__example_worker_process_zmq, args=(self.tokenizer, self), daemon=True
            )
            worker.start()
